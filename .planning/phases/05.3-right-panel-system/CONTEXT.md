...
### D10: 响应式宽度方案（三档断点）
- **1366px（最小笔记本）**：会话列表 220px / 对话区域 480px / 右侧面板 480px
- **1440px（主流笔记本）**：会话列表 220px / 对话区域 480px / 右侧面板 480px
  - 小于 1440px 时会话列表默认收起，展开时改为抽屉方式（浮在上方，不挤压整体空间）
- **1920px+（外接显示器）**：会话列表 240px / 对话区域 480px / 右侧面板 480px
- **实现**：使用 Tailwind 响应式断点 + `min-w-*` 约束 + JS 检测窗口宽度选择档位
- **拖拽范围**：最大不做限制，只要屏幕宽度剩余的宽度足够
