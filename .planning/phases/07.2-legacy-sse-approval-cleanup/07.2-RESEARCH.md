...
1. **Should active `docs/superpowers/*` be considered active docs or historical notes?**
   ...
   - What's unclear: Whether this repo treats `docs/superpowers` as active design docs or archived planning artifacts. [ASSUMED]
   - Recommendation: Planner should include a docs-cleanup task for `docs/superpowers/specs/2026-07-01-ag-ui-tool-approval-design.md`, `docs/superpowers/plans/2026-07-01-ag-ui-tool-approval.md`, and `docs/diagrams/permission-approval-current-design.excalidraw` if they are active. [ASSUMED]
...
2. **Is `ApprovalCard` mounted only through `AguiApprovalInterrupt`?**
   ...
   - What's unclear: Full app mounting location was not exhaustively mapped in this research excerpt. [ASSUMED]
   - Recommendation: Planner should add a targeted source check proving `AguiApprovalInterrupt` remains imported/mounted after App cleanup. [VERIFIED: `07.2-CONTEXT.md`]
