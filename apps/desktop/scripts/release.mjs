...
export async function smokeAgentSidecar({
  ...
  try {
    while (Date.now() - startedAt < timeoutMs) {
      ...
      if (closedResult) {
        throw new Error(`Agent sidecar exited during smoke (${closedResult.signal ?? `exit ${closedResult.code ?? 'unknown'}`}): ${output.trim()}`);
      }
      try {
    ...
    throw new Error(`Agent sidecar smoke timed out: ${output.trim()}`);
  } finally {
    if (!closed) {
      child.kill();
      await Promise.race([
