...
fn complete_environment() -> GoEnvironment {
    GoEnvironment::from_pairs(HashMap::from([
        ("PALADIN_DATABASE_URL".into(), "postgres://phase12-bootstrap-db".into()),
        ("PALADIN_REDIS_URL".into(), "redis://phase12-bootstrap-redis".into()),
...
async fn d05_explicit_import_accepts_only_complete_environment_and_masks_result() {
    ...
    let imported = manager.import_from_environment(&complete_environment()).await.unwrap();
    assert_eq!(imported.provenance, GoConfigProvenance::EnvironmentImport);
    assert!(!serde_json::to_string(&imported).unwrap().contains("phase12-bootstrap-jwt"));
}
...
async fn d06_partial_environment_is_diagnosed_without_persistence() {
    let dir = tempdir().unwrap();
    let manager = GoConfigManager::new_for_app_data(dir.path()).await.unwrap();
    let partial = GoEnvironment::from_pairs(HashMap::from([("PALADIN_DATABASE_URL".into(), "postgres://only".into())]));
...
async fn d08_clear_marks_local_handling_and_normal_bootstrap_never_resurrects_values() {
    let dir = tempdir().unwrap();
    let manager = GoConfigManager::new_for_app_data(dir.path()).await.unwrap();
    manager.save(GoConfigInput::from(complete_environment())).await.unwrap();
    manager.clear().await.unwrap();
    let bootstrap = manager.bootstrap_from_environment(&complete_environment()).await.unwrap();
...
async fn d07_ordinary_parent_environment_cannot_override_saved_config_but_marked_session_can() {
    ...
    let parent = GoEnvironment::from_pairs(HashMap::from([
        ("PALADIN_DATABASE_URL".into(), "postgres://phase12-parent-db".into()),
        ("PALADIN_REDIS_URL".into(), "redis://phase12-parent-redis".into()),
    ...
    let override_snapshot = manager.runtime_snapshot_for_marked_session(&parent, "PALADIN_GO_SESSION_OVERRIDE").await.unwrap();
    assert_eq!(override_snapshot.source, GoRuntimeSource::SessionOverride);
    assert!(!manager.load_masked_config().await.unwrap().fingerprint.contains("parent-jwt"));
}
