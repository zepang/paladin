...
业务配置名称包括：`OPENAI_API_KEY`、`OPENAI_BASE_URL`、`OPENAI_MODEL`、`DATABASE_URL`、`DATABASE_MAX_CONNS`、`PALADIN_AGENT_PORT`、`PALADIN_SERVER_PORT`。应用还会按 allowlist 转发必要的 home、临时目录、locale 和 TLS 证书发现变量。文档、诊断输出和 build manifest 只可出现变量名称，不可复制 secret、token、密码、DSN 或变量值。

启动时 Agent 与 Server 必须通过 readiness 探测后，主窗口才进入正常工作流。Go Server 依赖不可用时允许以 `降级（非阻塞）` 继续；核心 Agent 失败仍会阻塞启动并显示诊断。

## 日志与诊断

- macOS：`~/Library/Logs/dev.paladin.desktop/`
...
- `缺少必要配置：OPENAI_API_KEY`：退出应用，按部署渠道安全配置该变量后重新启动。不要把值粘贴到 issue、日志或聊天中。
